...
export function GearDeck() {
  ...
  // `.src` matters: the imports are StaticImageData objects now, and the
  // preloader assigns straight to `new Image().src` — an object there would
  // request "[object Object]" and fail behind the static burst, invisibly.
  const images = useMemo(() => setup.map((item) => item.image?.src ?? ""), []);
  ...
  return (
    <div onKeyDown={onKeyDown}>
      <div className="relative mx-auto w-full max-w-[320px]">
        ...
        >
          <GearCard item={item} number={index + 1} featured headingLevel="h3" />
        </div>
